    ...
    (newFiles: File[]) => {
      ...
      const validFiles = newFiles.filter((file) => {
        ...
        if (!acceptedFormats.includes(extension)) {
          useErrorStore.getState().setError(`File type ${extension} is not accepted.`);
          return false;
        }
        if (file.size > maxFileSize) {
          useErrorStore.getState().setError(`File ${file.name} is larger than 10MB.`);
          return false;
        }
        return true;
      });
      ...
      if (internalFiles.length + validFiles.length > maxFiles) {
        useErrorStore.getState().setError(`You can only upload up to ${maxFiles} files.`);
        return;
      }
    ...
    >
      <div className="h-full flex flex-col gap-4 sm:gap-6">
        {hasFiles && (
          <div className="flex flex-wrap gap-3">
            {internalFiles.map((file, index) => (
              <div
                key={index}
                className="bg-zinc-800 px-3 py-1 rounded-md flex items-center text-sm"
              >
        ...
        <label
          ...
          } items-center justify-center px-4 py-8 border border-dashed rounded-md self-start transition-colors ${
            isMaxFilesReached
              ? "border-zinc-700"
              : isDragging
              ? "border-emerald-700"
